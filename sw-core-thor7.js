...
const CACHE_NAME='thor-loterias-thor7-v143-cadastro-login-fix';
...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.allSettled(CORE.map(async url=>{
      try{
        const req=new Request(url+(url.includes('?')?'&':'?')+'_refresh=thor7-v143-cadastro-login-fix',{cache:'no-store'});
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    clients.forEach(c=>c.postMessage({type:'THOR_UPDATED',version:'THOR 7 V142',refresh:'thor7-v143-cadastro-login-fix'}));
  })());
});
