...
const CACHE_NAME='thor-loterias-thor7-v144-tela-antes-concurso';
...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.allSettled(CORE.map(async url=>{
      try{
        const req=new Request(url+(url.includes('?')?'&':'?')+'_refresh=thor7-v144-tela-antes-concurso',{cache:'no-store'});
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    clients.forEach(c=>c.postMessage({type:'THOR_UPDATED',version:'THOR 7 V144',refresh:'thor7-v144-tela-antes-concurso'}));
  })());
});
