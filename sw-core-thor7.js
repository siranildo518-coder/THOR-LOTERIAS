...
const CACHE_NAME='thor-loterias-thor7-v145-jogos-compactos';
...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.allSettled(CORE.map(async url=>{
      try{
        const req=new Request(url+(url.includes('?')?'&':'?')+'_refresh=thor7-v145-jogos-compactos-jogos-compactos',{cache:'no-store'});
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    clients.forEach(c=>c.postMessage({type:'THOR_UPDATED',version:'THOR 7 V145',refresh:'thor7-v145-jogos-compactos-jogos-compactos'}));
  })());
});
