(function(){
  ...
  async function copiarJogos(){
    ...
    if(!ok){
      var area=document.createElement('textarea');area.value=texto;area.setAttribute('readonly','');
    ...
    avisar(ok?'Jogos copiados. Cole no WhatsApp.':'Não foi possível copiar os jogos.');
    fecharMenu();
  }
  ...
  function montar(){
    ...
    menu.innerHTML='<button type="button" id="thorCompartilharC4" style="width:100%;padding:12px 10px;border:0;border-bottom:1px solid #31506c;background:transparent;color:#fff;text-align:left;font:700 12px Arial,sans-serif;">↗ Compartilhar</button><button type="button" id="thorCopiarJogosC6" style="width:100%;padding:12px 10px;border:0;background:transparent;color:#fff;text-align:left;font:700 12px Arial,sans-serif;">▣ Copiar jogos</button>';
    ...
    menu.querySelector('#thorCopiarJogosC6').addEventListener('click',copiarJogos);
