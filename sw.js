...
const CACHE_NAME='thor-loterias-base-142-gerador-abertura-fix-3';
...
function prepararHome(html){
  ...
  if(!html.includes('src="./gerador-palpites.js')) html=html.replace('</body>','<script src="./gerador-palpites.js?v=abertura-fix-3"><\/script></body>');
...
self.addEventListener('install',event=>{event.waitUntil((async()=>{const cache=await caches.open(CACHE_NAME);for(const url of CORE){try{const sep=url.includes('?')?'&':'?';const req=new Request(url+sep+'_refresh=gerador-abertura-fix-3',{cache:'no-store'});const res=await respostaAtualizada(req);if(res&&res.ok)await cache.put(url,res.clone())}catch(_){}}})());self.skipWaiting()});
self.addEventListener('message',event=>{if(event.data&&event.data.type==='SKIP_WAITING')self.skipWaiting()});
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>k!==CACHE_NAME).map(k=>caches.delete(k)));await self.clients.claim();const clients=await self.clients.matchAll({type:'window',includeUncontrolled:true});clients.forEach(c=>c.postMessage({type:'THOR_UPDATED',version:'142',refresh:'gerador-abertura-fix-3'}))})())});
